...
export const fetchFromOpenAI = async (query: string): Promise<any> => {
  try {
    ...
    const promptTemplate = `Ты — AI-ассистент, помогающий искать реальные товары в интернете (Zalando, Amazon, Ozon, Wildberries и другие магазины). На основе пользовательского запроса сгенерируй список из 3 карточек товаров в формате JSON.
    ...
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "Ты — помощник для поиска товаров. Отвечай только JSON."
          },
          {
            role: "user",
