...
export function generateBrandSuggestionPrompt(description: string): string {
  return `Ты эксперт по брендам и товарам. Назови 6 популярных брендов с конкретными товарами, которые могут соответствовать запросу: '${description}'. 
...
[
  ...
  {"brand": "Название бренда 6", "product": "Название товара 6", "description": "Описание товара 6"}
]`;
}
