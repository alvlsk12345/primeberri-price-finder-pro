...
export const SupabaseTab: React.FC = () => {
  console.log('[SupabaseTab] Начало рендера компонента SupabaseTab');
  
  try {
    const { checkingStatus, connectionStatus, handleCheckConnection } = useSupabaseConnection();
    console.log('[SupabaseTab] useSupabaseConnection загружен успешно');
    
    const { supabaseConfig, handleSupabaseBackendChange, handleFallbackChange } = useSupabaseConfig();
    console.log('[SupabaseTab] useSupabaseConfig загружен успешно');
    ...
    console.error('[SupabaseTab] Ошибка при рендеринге компонента:', error);
