...
export const BrandSuggestionList: React.FC<BrandSuggestionListProps> = ({
  ...
  return <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">Рекомендованные товары AI помощника</h2>
        {isLoading && <RefreshCw className="mr-2 h-4 w-4 animate-spin" />}
      </div>
      
      {isLoading ? <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-3">
          {[...Array(6)].map((_, i) => <div key={i} className="border rounded-md overflow-hidden">
        ...
        </div> : error ? <div className="p-4 bg-red-50 border border-red-200 rounded-md flex items-center gap-3">
        ...
        </div> : suggestions.length === 0 ? <div className="text-center py-4">
          <p className="text-gray-500">Нет предложений по вашему запросу. Попробуйте изменить описание.</p>
        </div> : <ScrollArea className="h-full max-h-[600px] pr-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-3 pb-2">
            {suggestions.map((suggestion, index) => <BrandSuggestionItem key={index} suggestion={suggestion} onSelect={immediate => handleSelectProduct(suggestion, immediate)} index={index} />)}
          </div>
        </ScrollArea>}
    </div>;
};
